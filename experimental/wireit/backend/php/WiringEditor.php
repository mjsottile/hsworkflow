...
class WiringEditor {
    ...
    public function saveWiring($language, $name, $working) {
    	...
    	$result = $this->query( sprintf("SELECT * from wirings where name='%s' AND language='%s'", ($name), ($language)) );
    ...
    public function loadWiring($language, $name) {
       $wirings = $this->queryToArray( sprintf("SELECT * from wirings WHERE name='%s' AND language='%s'", ($name), ($language)) );
       return $wirings[0];
    }

    public function deleteWiring($language, $name) {
      $this->query( sprintf("DELETE from wirings WHERE name='%s' AND language='%s'", ($name), ($language)) );
      return true;
   }
