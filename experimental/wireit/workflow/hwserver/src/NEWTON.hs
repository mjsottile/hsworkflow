...
newtonworkflow [n] =
           ...
           feval (dbldatum:_) =
               let 
                   ...
                   result =    if (reldiff f f1) > 0.00000000000005 then
                                   [ toDoubleDatum f1, toMissingDatum ]
                               else
                                   [ toMissingDatum, toDoubleDatum f1 ]
               in
                   result
           feval _ = [] 
    ...
    in do
        ...
        makeFunction feval  ["impipe1"] ["impipe1","impipe2"]

        makeTake "impipe2" "OutPipe" 1
...
main =
        do
          putStrLn "newton"
          (Just s0) <- startWorkflow (newtonworkflow [(toIntDatum 3)])
          (sn, [ output ]) <- runWorkflow s0
